...
export default {
  ...
  methods: {
    ...
    handleTouchStart(event, lineIndex, charIndex) {
      this.touchStartIndex = { line: lineIndex, char: charIndex };
    ...
    handleTouchMove(event) {
      ...
      if (element && element.dataset.line !== undefined) {
        ...
        this.extendSelection(lineIndex, charIndex);
      }
    },
    handleTouchEnd() {
    ...
    extendSelection(lineIndex, charIndex) {
      ...
      // Для кросс-строчного выделения потребуется более сложная логика
    },
    handleSelection() {
